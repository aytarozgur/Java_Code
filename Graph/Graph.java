...
public class Graph {
	...
	public void dijkstra(String startName) {
		...
		start.weightedCostFromStartVertex = 0;
		start.numEdgesFromStartNode = 0;
		//create a PriorityQueue of Path object
		...
		while (!pathQueue.isEmpty()){
			...
			//visit every edge adjacent to vertex current
			for (Edge e:current.adjacent){
				Vertex neighberVertex = e.dest;
				double neighberWeight = e.cost;
				double weightFromCurrent = current.weightedCostFromStartVertex + neighberWeight;

				//if found a shorter path, update vertex information
				if(weightFromCurrent < neighberVertex.weightedCostFromStartVertex){
					neighberVertex.weightedCostFromStartVertex = weightFromCurrent;
					neighberVertex.numEdgesFromStartNode = current.numEdgesFromStartNode + 1;
					neighberVertex.prev = current;
					Path newPath = new Path(neighberVertex, weightFromCurrent);
	...
	public void findAllPaths(boolean weighted) {
		...
		for(String vertexName: vertices.keySet()){
			...
			//set numVertexConnected, totalUnweightedPathLengh/totalWeightedPathLength to 0;
			v.clearPathInfo();
			//check to see whether it's weighted graph or unweighted graph
			if(weighted) 
				dijkstra(vertexName);
			else 
				findUnweightedShortestPath(vertexName);

			//loop through the neighboring vertex nodes
			for (Vertex node : vertices.values()){
				//if this vertex has not been visited 
				if(node != v && node.weightedCostFromStartVertex != INFINITY){

					//get path from current node to dest vertex
					Path currentPath = getPath(node.name);
					//update the longest path
					if (currentPath.length() > longest.length())
						longest = currentPath;

					v.numVertexConnected++;
					v.totalWeightedPathLength += node.weightedCostFromStartVertex;
					v.totalUnweightedPathLengh = longest.length();
				}
			}
		}
	}
	...
	// Used in the Dijsktra method and to track the longest shortest path
