...
public class FootballRanker {
    ...
    /**
     * Calculate top teams based solely on average path length of unweighed shortest paths.
    ...
    private double printRootMeanSquareError(List<String> humanRanks,
        ...
        int numOfTeams = humanRanks.size();
        double result = 0;
        //System.out.println(humanRanks);
        List<String> predictedRank = new ArrayList<String>();
        for(AllPathsInfo singlePath: paths){
        	predictedRank.add(singlePath.getName());
        }
        for (String name:humanRanks){
        	int realRank = humanRanks.indexOf(name)+1;
        	int predRank = predictedRank.indexOf(name) +1;
        	result += (realRank -predRank) *(realRank - predRank);
        	System.out.printf("%-20s - actual rank:%-12d predicted rank:%12d\n", name,realRank,predRank);
        }
        result = Math.sqrt(result/numOfTeams);
        double resultRoundOff = Math.rint(result*10)/10;
        Math.round(resultRoundOff);
        System.out.println("Root Mean Square Error: " + resultRoundOff);
